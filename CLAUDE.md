...
**每次有意义的改动后必须立即 commit**：写完文件、修完 bug、完成一个功能步骤后，立即 `git add` + `git commit`，不要积攒。
...
**后端改动后必须自动重启**，不要问用户手动重启。判断依据：
- 修改了 `backend/` 下的 `.py` 文件 → 自动重启后端
- 添加了一次性迁移（如 `init()` 里的修复逻辑）→ 必须重启使迁移生效
- 重启方式：先 kill 旧进程（`taskkill`），再 `uv run python main.py`（后台启动）

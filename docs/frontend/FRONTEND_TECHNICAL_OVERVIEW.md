...
> 最后更新：2026-06-01 · 分支 `feat/pdf-reader-refactor`
...
│
├── components/             # 18 个顶层组件 + 5 个 PDF 子组件
...
│   ├── SourcePanel.vue     #   引用源面板：水平滚动源卡片列表
│   ├── LibraryPanel.vue    #   文献库面板：拖拽上传、搜索、排序、回收站、批量操作
│   ├── LibraryPaperCard.vue#   文献卡片：标题高亮、关键词标签、操作按钮
│   ├── TrashPanel.vue      #   回收站面板：已删论文列表、恢复/永久删除
│   ├── ImportQueueList.vue #   导入队列：进度条、重试/移除按钮
│   ├── DeleteConfirmDialog.vue # 删除确认弹窗（支持批量 + 跳过确认）
...
│   ├── use-drop-zone.ts    #   拖拽区域：嵌套计数防闪烁、过滤 .pdf 文件
│   ├── use-auto-scroll.ts  #   自动滚动：监听 trigger 变化自动滚到底部
│   ├── use-file-upload.ts  #   文件上传：创建隐藏 input 触发文件选择
│   ├── use-session-manager.ts # 会话管理：切换/删除会话、消息映射
│   └── use-citation-preview.ts # 引用预览：悬停延迟、视口夹紧、WPS 跳转回退
│
├── router/
│   └── index.ts            #   路由懒加载：/ → ChatView，/settings → SettingsView
│
├── services/               # API 层（5 个模块）
│   ├── api-client.ts       #   基础 HTTP 客户端：buildApiUrl、requestJson、apiRequest
│   ├── sse-client.ts       #   SSE 流式客户端：postAskStream（8 种事件类型）
│   ├── assistant-api.ts    #   WPS 上下文同步：选区、文档内容
│   ├── conversation-api.ts #   会话 CRUD：创建、列表、删除、获取消息
│   └── library-api.ts      #   文献管理：列表、删除、导入、回收站、状态轮询
│
├── stores/                 # 4 个 Pinia Store
│   ├── ui.ts               #   UI 状态：侧边栏、PDF 阅读器开关
│   ├── conversation.ts     #   对话核心：消息流、SSE 处理、会话管理（~332 行）
│   ├── library.ts          #   文献库：论文列表、回收站、导入队列（~517 行）
│   └── settings.ts         #   设置：API 配置、模型列表、功能开关
│
├── types/
│   ├── content.ts          #   ContentBlock 类型（段落/标题/代码/列表/表格等）
│   ├── source.ts           #   SourceCard 类型（引用源信息）
│   ├── paper.ts            #   PaperItem、ImportStartResult、ImportStatus、ImportProgressEvent
│   ├── conversation.ts     #   ConversationSession、ConversationMessage
│   └── message.ts          #   ConversationStatus、UserMessage、AssistantMessage、ConversationRecord
│
├── utils/
│   ├── markdown-inline.ts  #   行内 Markdown 渲染（加粗/斜体/代码）
│   └── citation-renderer.ts #  引用渲染：XSS 转义 + 行内引用标记生成
│
├── views/
│   ├── ChatView.vue        #   主视图：编排交互逻辑（~412 行，已拆分 composables）
...
| **SourcePanel** | `SourcePanel.vue` | 引用源面板：水平滚动卡片列表，展示标题、页码、章节 | `sources: SourceCard[]` | `open-source(source)` |
| **LibraryPanel** | `LibraryPanel.vue` | 文献库面板：拖拽上传、Fuse.js 搜索 + 年份/作者筛选 + 排序、全选（半选态）、回收站视图、批量删除确认、导入队列 | `papers`, `loading`, `error`, `selectedIds` | `toggle`, `upload`, `remove`, `select-all`, `retry` |
...
| **TrashPanel** | `TrashPanel.vue` | 回收站面板：已删论文列表、恢复、永久删除（带内联确认） | `papers: PaperItem[]`, `loading: boolean` | `restore(id)`, `permanent-delete(id)`, `back()` |
| **ImportQueueList** | `ImportQueueList.vue` | 导入队列：每项进度条、完成/错误状态、重试/移除按钮 | `items: ImportQueueItem[]`, `isEmptyLibrary: boolean` | `retry(idx)`, `remove(idx)` |
| **DeleteConfirmDialog** | `DeleteConfirmDialog.vue` | 删除确认弹窗：支持批量 N 篇、勾选"本次会话不再询问" | `visible`, `title`, `count`, `skipConfirm` | `confirm`, `cancel`, `update:skipConfirm` |
...
| 视图 | 文件 | 职责 |
|------|------|------|
| **ChatView** | `views/ChatView.vue` | 主应用视图（~412 行）。编排：消息发送、WPS 轮询集成、Demo 模式自动激活。已将会话管理、引用预览、自动滚动、文件上传等逻辑拆分到 composables。快捷键：Ctrl+K 打开历史、Ctrl+N 新建对话 |
...
| **useDropZone** | `use-drop-zone.ts` | 拖拽区域状态（`dragActive`），嵌套元素 dragenter/dragleave 计数防闪烁，过滤只接受 `.pdf` 文件 | 否 |
| **useAutoScroll** | `use-auto-scroll.ts` | 监听一组 `WatchSource` 触发器，变化时通过 `nextTick` 自动滚动容器到底部 | 否 |
| **useFileUpload** | `use-file-upload.ts` | 程序化创建隐藏 `<input type="file">`，支持 `accept` 和 `multiple` 选项，触发系统文件选择器 | 否 |
| **useSessionManager** | `use-session-manager.ts` | 会话列表加载、切换会话（解析 `blocks_json`/`sources_json` → `ConversationRecord`）、删除会话，支持 Demo 模式回退 | 是 → conversation-api |
| **useCitationPreview** | `use-citation-preview.ts` | 引用预览交互：300ms 悬停延迟、视口边界夹紧定位、点击跳转（WPS ShellExecute → PDF Reader 回退） | 否（跳转委托 ui store） |
...
### 6.2 `conversation` Store（`stores/conversation.ts`，~332 行）
...
### 6.3 `library` Store（`stores/library.ts`，~517 行）

**职责**：文献库管理——论文列表、回收站、导入管道
...
| `trashedPapers` | `PaperItem[]` | 回收站论文列表 |
...
| `removeQueueItem(index)` | 移除导入队列项 |
| `clearImportError/Queue()` | 清理导入状态 |
| `loadTrashedPapers()` | 加载回收站列表 |
| `restorePaperFromTrash(id)` | 从回收站恢复论文 |
| `permanentDeleteFromTrash(id)` | 永久删除论文 |
...
| `apiRequest<T>(pathname, init?)` | 通用 JSON 请求，直接返回 `T`（不解包 envelope），用于新接口 |
...
| `deletePaper(paperId)` | DELETE | `/api/v1/library/items/{id}` | 移入回收站 |
| `retryImport(paperId)` | POST | `/api/v1/library/items/{id}/retry` | 返回 `ImportStartResult` |
...
| `fetchTrashedPapers()` | GET | `/api/v1/library/trash` | 获取回收站列表 |
| `restorePaper(paperId)` | POST | `/api/v1/library/items/{id}/restore` | 从回收站恢复 |
| `permanentDeletePaper(paperId)` | DELETE | `/api/v1/library/items/{id}/permanent` | 永久删除 |
...
### 8.3 `types/paper.ts` — 论文与导入类型

```typescript
interface PaperItem {
  paper_id: string; title: string; authors: string; year: number | null
  keywords: string[]; file_path: string; file_hash: string
  chunk_count: number; total_pages: number; import_time: string
  status: string; library_item_id: string; kind: string; file_size: number
}
interface ImportStartResult { task_id: string; status: string }
interface ImportStatus { task_id: string; status: string; step: string; percent: number; error?: string }
interface ImportProgressEvent { step: string; percent: number }
```

### 8.4 `types/conversation.ts` — 会话类型

```typescript
interface ConversationSession { session_id: string; title: string; created_at: string; updated_at: string }
interface ConversationMessage { session_id: string; role: string; content: string; created_at: string; sources_json?: string; blocks_json?: string }
```

### 8.5 `types/message.ts` — 消息类型

```typescript
type ConversationStatus = 'idle' | 'requesting' | 'thinking' | 'streaming' | 'done' | 'error'
interface UserMessage { id: string; role: 'user'; content: string; createdAt: string }
interface AssistantMessage { id: string; role: 'assistant'; createdAt: string; thinking?: string; thinkingTimeMs?: number; blocks: ContentBlock[]; sources: SourceCard[]; streamingText?: string }
type ConversationRecord = UserMessage | AssistantMessage
```
...
| `NumberedSource` | `citation-renderer.ts` | `{ source: SourceCard, num: number }` |
...
路由定义：`/` → ChatView，`/settings` → SettingsView，`/:pathMatch(.*)*` → 重定向到 `/`。
...
| 导入队列 | 持久化到 localStorage，页面刷新后自动恢复，支持重试和移除队列项 |
| 导入重试 | 失败项可单独重试 |
...
| 排序 | 相关性 / 导入时间 / 年份 / 标题 |
| 搜索高亮 | 匹配词在标题中高亮显示 |
| 全选/批量操作 | 全选 + 批量删除（带确认对话框，支持"本次不再询问"） |
| 单选 | 勾选论文用于对话时引用 |
| 预览 | 悬停卡片可快速预览 |
| 删除 | 单个或批量删除，移入回收站 |
| 回收站 | 独立视图查看已删论文，支持恢复和永久删除 |
| 永久删除 | 带内联二次确认，防止误操作 |
...
| DELETE | `/api/v1/library/items/{id}` | 删除论文（移入回收站） | library store |
| POST | `/api/v1/library/items/{id}/retry` | 重试导入 | library store |
| POST | `/api/v1/library/items/{id}/restore` | 从回收站恢复 | library store |
| DELETE | `/api/v1/library/items/{id}/permanent` | 永久删除 | library store |
| GET | `/api/v1/library/trash` | 回收站列表 | library store |

...
> 最后更新: 待核实 | 注：部分链接可能指向已迁移的文件，以实际目录为准
...
`archives/_arch_doc/` （已不存在）—— 原归档内容的设计意图已被吸收到当前文档，归档目录本身不再保留。

| 原归档来源 | 当前处理方式 | 结论 |
|---|---|---|
| `archives/_arch_doc/流程架构图.md`（已不存在） | 吸收其中还能成立的主链路表达 | 已采纳三层主链、导入链、对话链；旧细节实现不继承 |
| `archives/_arch_doc/Decision-Snapshot/`（已不存在） | 用作”代码曾经怎么长成这样”的证据 | 部分采纳，用于现状映射和迁移判断，不直接约束新架构 |
| `archives/_arch_doc/Short-Iteration/`（已不存在） | 只保留迭代痕迹 | 不再作为活动计划来源 |
| `archives/_arch_doc/开发文档/`（已不存在） | 只吸收仍然成立的业务意图和接口背景 | 旧 API 命名、旧目录边界、旧解析主链大多废弃 |
| `archives/_arch_doc/讨论/`（已不存在） | 吸收用户已确认的设计意图 | 已采纳三源加权、双循环、异常容灾、compact、进程内内存运行态策略 |
| `archives/_arch_doc/plan.md`（已不存在） | 视为历史版本计划 | 不再直接驱动当前重构 |
...
- [文档导入与预处理模块](./data_layer_doc/__achive__/文档导入与预处理模块/README.md)
- [PDF 清洗重构方案](./data_layer_doc/__achive__/关于PDF的清洗模块/PDF清洗重构方案.md)
- ~~[自研清洗逻辑 VS MinerU](../../../我的能力树/能力树/RAG相关/对于pipeline/_自研清洗逻辑_VS_MinerU_.md)~~ （已不存在）
...
- [现状代码映射](./代办/_现状代码映射.md)
- [后端文件分布重规划](./代办/_后端文件分布重规划.md)
- [参考源码借鉴清单](./代办/_参考源码借鉴清单.md)
...
- [docs/backend/代办/后端架构待办.md](./代办/后端架构待办.md)

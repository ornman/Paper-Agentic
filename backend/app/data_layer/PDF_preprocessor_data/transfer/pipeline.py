...
class PipelineState:
    ...
    has_formula_signals: bool = False
...
def decide_route(
    ...
    if is_scan_like:
        return Route.E
    if has_formula_signals and has_form_fields and has_images:
        return Route.D
    if has_formula_signals or has_form_fields or has_table_signals:
        return Route.C
    if has_images:
        return Route.B
    return Route.A
...
class PipelineOrchestrator:
    ...
    async def run(
        ...
        try:
            # 阶段 1：探针
            await self._run_probe(state)

            # 阶段 2：路由决策
            self._run_routing(state)
    ...
    async def _run_probe(self, state: PipelineState):
        """执行探针（仅 PDF，非 PDF 文件跳过探针由 MinerU 直接处理）"""
        state.stage = PipelineStage.PROBING
        self._emit(state, "probe.started", "开始探针检测")

        if state.file_path.suffix.lower() != ".pdf":
            # DOCX/DOC/PPTX/XLSX 等：MinerU 直接解析，跳过 PDF 探针
            state._probe_result = None
            state.has_images = False
            state.has_form_fields = False
            state.has_formula_signals = False
            state.is_scan_like = False
            self._emit(state, "probe.skipped", f"非 PDF 文件 ({state.file_path.suffix})，跳过探针")
            return

        from ..probe import probe_pdf
        probe_result = probe_pdf(state.file_path)

        state._probe_result = probe_result
        state.has_images = probe_result.has_images
        state.has_form_fields = probe_result.has_form_fields
        state.has_formula_signals = probe_result.has_formula_signals
        state.is_scan_like = probe_result.is_scan_like

        self._emit(state, "probe.completed", "探针完成", {
            "has_images": probe_result.has_images,
            "has_form_fields": probe_result.has_form_fields,
            "has_formula_signals": probe_result.has_formula_signals,
            "is_scan_like": probe_result.is_scan_like,
            "has_table_signals": probe_result.has_table_signals,
            "complexity": probe_result.doc_complexity_level,
            "recommended_route": probe_result.recommended_route,
        })

    def _run_routing(self, state: PipelineState):
        """路由决策"""
        state.stage = PipelineStage.ROUTING

        probe_result = getattr(state, "_probe_result", None)
        if probe_result:
            route = decide_route(
            ...
            route = decide_route(
                has_text_layer=True,
                is_scan_like=state.is_scan_like,
                has_images=state.has_images,
                has_form_fields=state.has_form_fields,
                has_formula_signals=state.has_formula_signals,
                has_table_signals=False,
            )

        state.route = route
        self._emit(state, "routing.decision", f"路由决策: {route.value}", {
            "route": route.value,
        })
    ...
    async def _run_transformation(self, state: PipelineState, output_dir: Path | None):
        ...
        # ── 关键优化：图片一到手就启动 VLM ──
        # 只要 MinerU metadata 里有 image_paths，或 route 为 B/C/D 且有图片，就触发 VLM
        mineru_image_paths = result.mineru_metadata.get("image_paths", [])
        vlm_task = None
        has_vlm = bool(mineru_image_paths) or (result.images and state.route in (Route.B, Route.C, Route.D))
        if has_vlm:
